...
% =========================================================
%  ★ 0. 全局字体设置：让坐标轴刻度默认变为 Times New Roman
% =========================================================
...
% Colorbar 字体设定
c = colorbar; 
c.Label.String = '\fontname{Times New Roman}Power (dB)';
c.Label.Interpreter = 'tex';
c.Label.FontWeight = 'bold';

% X轴和Y轴中英文混排
xlabel('\fontname{SimSun}时间 \fontname{Times New Roman}(ms)', 'FontWeight', 'bold', 'FontSize', 12);
ylabel('\fontname{SimSun}频率 \fontname{Times New Roman}(MHz)', 'FontWeight', 'bold', 'FontSize', 12);

% Title 中英文混排处理（由于使用 tex 解释器，文件名中的下划线需转义）
[~, fname_only, ~] = fileparts(test_file);
safe_fname = strrep(fname_only, '_', '\_');
title_str = sprintf('\\fontname{SimSun}特征提取：\\fontname{Times New Roman}%s (%g ms - %g ms) | Downsample: x%d', safe_fname, t_start_ms, t_start_ms + t_duration_ms, ds_factor);
...
if isempty(stats)
    % 纯中文文本
    text(t_start_ms + t_duration_ms/2, 0, '\fontname{SimSun}未检测到任何信号块', 'Color', 'w', 'FontSize', 14, 'HorizontalAlignment', 'center');
else
    for k = 1:length(stats)
        ...
        if f_len_box < bw_threshold
            ...
            % 纯英文批注强制设定为 Times New Roman
            text(t_start_box + t_len_box, f_start_box + f_len_box/2, ...
                sprintf(' BW:%.1fMHz', f_len_box), 'Color', color, 'FontSize', 9, 'FontWeight', 'bold', 'HorizontalAlignment', 'left', 'FontName', 'Times New Roman');
            
            text(t_start_box + t_len_box/2, f_start_box - 1.0, ...
                sprintf('Dur:%.1fms', t_len_box), 'Color', color, 'FontSize', 9, 'FontWeight', 'bold', 'HorizontalAlignment', 'center', 'FontName', 'Times New Roman');
            ...
            text(t_start_box, f_start_box - 2.0, sprintf('Video BW:%.1fMHz', f_len_box), ...
                 'Color', color, 'FontSize', 10, 'FontWeight', 'bold', 'BackgroundColor', [1 1 1 0.7], 'FontName', 'Times New Roman');
        end
    end
end
...
if size(hop_centroids, 1) > 1
    ...
    for k = 1:size(hop_centroids,1)-1
        ...
        % 纯英文批注强制设定为 Times New Roman
        text((x1+x2)/2, (y1+y2)/2, int_str, 'Color', 'k', 'BackgroundColor', 'y', 'FontSize', 9, 'FontWeight', 'bold', 'HorizontalAlignment', 'center', 'FontName', 'Times New Roman');
    end
end
